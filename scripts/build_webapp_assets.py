...
def build_abstract(stats: dict, agg_df: pd.DataFrame, nstar_df: pd.DataFrame) -> dict:
    ...
    agree_10 = format_float(n10["bottom_decile_median"].iloc[0]) if not n10.empty else None
    agree_50 = None
...
def main() -> None:
    ...
    nstar_df = load_csv(repo_root, "outputs/step3_nstar_mae.csv")
    ...
    protocol_curves = {}
    for protocol, group in step3_summary.groupby("protocol"):
        protocol_curves[protocol] = group.sort_values("N").to_dict(orient="records")
    ...
    payload = {
        ...
        "protocols": step3_protocol.to_dict(orient="records"),
        "protocol_curves": protocol_curves,
    }

    output_path = os.path.join(repo_root, "outputs", "webapp_data.json")
    with open(output_path, "w", encoding="utf-8") as handle:
        json.dump(payload, handle, indent=2)

    print("Webapp data written to outputs/webapp_data.json")
